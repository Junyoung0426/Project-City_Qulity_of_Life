...
## Dahboard file Explaination
This is the Dashboard. I put the Histogram, Biplot, MDS Plot and Parrallel_Coordinates. Also, I put the new chart which we learned in the lecture, Chord Diagram. In my dashboard, I can select the attributes which I want to know. Firstl, I will select the one variable, “Housing”.
Also, I used the brush in d3 on the Biplot to highlight the Points which I want to know. If I brush the scttor plot, I can find the name of city which I dragged with mouth in console. Also, by this I can link it to the MDS Plot. However, There is some problem and no idea to link well by using the brush. Even if I failed to connect all chart when using the brush, I have a good story of my dashboard. Firstly, If I choose the dots in biplot, I could highlight the dots and connect to the MDS plot and Parallel plot. Therefore, we can know where the dot located on MDS and Parallel Coordinated plot, and get a good data visualization. In MDS plot, the city which i choose in Biplot wil highlighted on the MDS plot. For the parallel Coordinates, only the city which I choose will appear in the Parallel plot and see the good visualiztion and get approximately accurate data. However, I failed to make it through d3… It will be good visualization to get the reasonable city which people hope to live.
